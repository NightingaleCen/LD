...
class MultiLDHead(GFLHead):
    ...
    def __init__(self,
                 ...
                 loss_CA_MKD=dict(
                     type='SinkhornDistance', loss_weight=0.25, T=10),
                 imitation_method='gibox',
                 **kwargs):
        super(MultiLDHead, self).__init__(num_classes, in_channels, **kwargs)
        ...
        self.loss_CA_MKD = build_loss(loss_CA_MKD)
    ...
    def loss_single(self, anchors, cls_score, bbox_pred, labels, label_weights,
                    bbox_targets, stride, soft_targets, soft_labels, x,
        ...
        soft_targets = [soft_target.permute(0, 2, 3,
                                            1).reshape(-1,
                                                       4 * (self.reg_max + 1)) for soft_target in soft_targets]

        soft_labels = [soft_label.permute(0, 2, 3,
                                          1).reshape(-1, self.cls_out_channels) for soft_label in soft_labels]

        teacher_x = [x.permute(0, 2, 3, 1).reshape(-1, 256) for x in teacher_x]
        ...
        loss_im = []
        for teacher_ind in range(num_teacher):
            if self.imitation_method == 'gibox':
                gi_idx = self.get_gi_region(soft_labels[teacher_ind], cls_score, anchors,
                                            bbox_pred, soft_targets[teacher_ind], stride)
                gi_teacher = teacher_x[teacher_ind][gi_idx]
                gi_student = x[gi_idx]

                loss_im.append(self.loss_im(gi_student, gi_teacher))
            elif self.imitation_method == 'decouple':
                fg_inds = (im_region > 0).nonzero().squeeze(1)
                ng_inds = (im_region == 0).nonzero().squeeze(1)
                if len(fg_inds) > 0:
                    loss_im.append(self.loss_im(x[fg_inds],
                                                teacher_x[teacher_ind][fg_inds]) + 2 * self.loss_im(
                        x[ng_inds], teacher_x[teacher_ind][fg_inds]))
                else:
                    loss_im.append(bbox_pred.sum() * 0)
            else:
                fg_inds = (im_region > 0).nonzero().squeeze(1)
                if len(fg_inds) > 0:
                    loss_im.append(
                        self.loss_im(
                            x[fg_inds],
                            teacher_x[teacher_ind][fg_inds]))
                else:
                    loss_im.append(bbox_pred.sum() * 0)

        loss_im = torch.stack(loss_im, dim=0)
        ...
        if len(pos_inds) > 0:
            ...
            soft_corners = []
            for teacher_ind in range(num_teacher):
                pos_soft_targets = soft_targets[teacher_ind][pos_inds]
                soft_corners.append(
                    pos_soft_targets.reshape(-1, self.reg_max + 1))
            ...
            # ld loss & CA-MKD
            loss_ld = torch.stack([self.loss_ld(
                pred_corners,
                soft_corners[teacher_ind],
                weight=weight_targets[:, None].expand(-1, 4).reshape(-1),
                avg_factor=4.0) for teacher_ind in range(num_teacher)], dim=0)
            loss_kd = torch.stack([self.loss_kd(
                cls_score[pos_inds],
                soft_labels[teacher_ind][pos_inds],
                weight=label_weights[pos_inds],
                avg_factor=pos_inds.shape[0]) for teacher_ind in range(num_teacher)], dim=0)

            # calculate CA-MKD weights
            target_distribution = F.one_hot(
                target_corners.long(),
                num_classes=self.reg_max + 1).float()
            loss_ld_teacher = torch.stack([self.loss_CA_MKD(
                soft_corners[teacher_ind],
                target_distribution,
                avg_factor=4.0) for teacher_ind in range(num_teacher)], dim=0).sum(dim=1)

            loss_kd_teacher = torch.stack([self.loss_cls(
                soft_labels[teacher_ind],
                (labels, score),
                weight=label_weights,
                avg_factor=num_total_samples) for teacher_ind in range(num_teacher)], dim=0)

            ld_weight = (1.0 - F.softmax(loss_ld_teacher, dim=0)
                         ) / (num_teacher - 1)
            kd_weight = (1.0 - F.softmax(loss_kd_teacher, dim=0)
                         ) / (num_teacher - 1)

            loss_ld = torch.mul(ld_weight, loss_ld).mean()
            loss_kd = torch.mul(kd_weight, loss_kd).mean()
            loss_im = torch.mul(kd_weight, loss_im).mean()
        ...
        if len(remain_inds) > 0:
            neg_pred_corners = bbox_pred[remain_inds].reshape(
                -1, self.reg_max + 1)
            neg_soft_corners = [soft_targets[teacher_ind][remain_inds].reshape(
                -1, self.reg_max + 1) for teacher_ind in range(num_teacher)]

            remain_targets = vlr_region[remain_inds]

            loss_ld_vlr = torch.stack([self.loss_ld_vlr(
                neg_pred_corners,
                neg_soft_corners[teacher_ind],
                weight=remain_targets[:, None].expand(-1, 4).reshape(-1),
                avg_factor=16.0) for teacher_ind in range(num_teacher)], dim=0).mean()  # TODO: 用CA-MKD替换？
            loss_kd_neg = 0 * self.loss_kd(
                cls_score[remain_inds],
                soft_labels[0][remain_inds],
    ...
    def loss(self,
        ...
        soft_targets = []
        reshaped_teacher_x = []
        for scale_ind in range(len(cls_scores)):
            soft_label = []
            soft_target = []
            single_scale_x = []
            for single_out_teacher in soft_teacher:
                soft_label.append(single_out_teacher[0][scale_ind])
                soft_target.append(single_out_teacher[1][scale_ind])
            for i in teacher_x:
                single_scale_x.append(i[scale_ind])
            soft_labels.append(soft_label)
            soft_targets.append(soft_target)
            reshaped_teacher_x.append(single_scale_x)
        teacher_x = reshaped_teacher_x
    ...
    def get_gi_region(self, soft_label, cls_score, anchors, bbox_pred,
        ...
        # difference between teacher score and student score on the whole
        # locations.
        z = teacher_score - student_score
